...
def ensure_dataset():
    if not os.path.exists(DATASET_PATH):
        create_realistic_dataset(500, 500)  # quick init, can be increased later
...
# ==========================================
# 3. MODEL TRAINING & LOADING (pickle)
# ==========================================
def train_model():
...
if analyze_btn and url_input:
    with st.spinner("Analyzing..."):
        ...
        with res_col2:
            ...
            df_impact = pd.DataFrame(filtered, columns=["Feature", "Impact"])
            st.bar_chart(df_impact.set_index("Feature"), height=250, color="#4f46e5")
            st.markdown('</div>', unsafe_allow_html=True)
