...
<style>
    ...
    .stTextInput > div > div > input, .stTextArea > div > div > textarea { background-color: #0f172a !important; border: 1px solid #1e293b !important; color: #e2e8f0 !important; border-radius: 12px !important; padding: 1rem !important; font-family: 'JetBrains Mono', monospace !important; font-size: 14px !important; }
    .stTextInput > div > div > input:focus, .stTextArea > div > div > textarea:focus { border-color: #4f46e5 !important; }
...
st.markdown("""
    <div class="ts-card" style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div style="text-align: right;">
            ...
            <span class="hero-tag">10K+ URLs</span>
        </div>
    </div>
""", unsafe_allow_html=True)
    ...
    if analyze_btn and url_input:
        with st.spinner("Analyzing..."):
            ...
            with res_col1:
                ...
                if feats.get('spam_keyword_count') > 0: indicators.append(f"Contains {feats.get('spam_keyword_count')} spam/urgent keywords.")
            ...
            with res_col2:
                ...
                df_impact = pd.DataFrame(list(impact_data.items()), columns=['Feature', 'Impact'])
                df_impact = df_impact[df_impact['Impact'] > 5].sort_values('Impact', ascending=True)
                if df_impact.empty: df_impact = pd.DataFrame([{"Feature": "Baseline Safe", "Impact": 10}])

                fig = px.bar(df_impact, x="Impact", y="Feature", orientation='h')
                fig.update_traces(marker_color=['#10b981' if val < 40 else '#f59e0b' if val < 70 else '#f43f5e' for val in df_impact['Impact']], marker_line_width=0, opacity=0.9)
                fig.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font_color='#94a3b8', margin=dict(l=0, r=0, t=0, b=0), height=250, xaxis=dict(showgrid=True, gridcolor='#1e293b', zeroline=False), yaxis_title=None, xaxis_title=None)
...
with tab2:
    ...
    if process_batch and batch_input.strip():
        ...
        st.dataframe(df_batch.style.map(lambda x: 'color: #f87171; font-weight:bold;' if x == 'MALICIOUS' else 'color: #34d399; font-weight:bold;' if x == 'SAFE' else ''), use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    def render_metric(col, title, value, color):
        col.markdown(f'<div class="ts-card" style="padding: 1.5rem;"><div class="uppercase-label">{title}</div><div style="font-size: 2rem; font-weight: 700; color: {color};">{value}</div></div>', unsafe_allow_html=True)
    
    render_metric(m1, "Accuracy", "98.2%", "#60a5fa")
    render_metric(m2, "Precision", "97.8%", "#34d399")
    render_metric(m3, "Recall", "98.5%", "#fbbf24")
    render_metric(m4, "F1 Score", "98.1%", "#c084fc")
    
    p_col1, p_col2 = st.columns(2)
    with p_col1:
        st.markdown('<div class="ts-card">', unsafe_allow_html=True)
        st.markdown("### 🗄️ Training Distribution")
        st.caption("Class balance in the current training dataset.")
        fig_pie = px.pie(names=['Safe', 'Malicious'], values=[5500, 5000], hole=0.6, color_discrete_sequence=['#10b981', '#f43f5e'])
        fig_pie.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', margin=dict(t=0, b=0, l=0, r=0), height=300, font_color="white", showlegend=False)
        st.plotly_chart(fig_pie, use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
        
    with p_col2:
        st.markdown('<div class="ts-card" style="height: 100%;">', unsafe_allow_html=True)
        st.markdown("### ⏱️ Training Lifecycle")
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("""
        <div style="background: rgba(15, 23, 42, 0.5); padding: 20px; border-radius: 16px; border: 1px solid #1e293b; margin-bottom: 16px;">
            <div class="uppercase-label">Total Samples</div>
            <div style="font-size: 24px; font-weight: bold;">10,500 <span style="font-size:12px; color:#34d399;">(Verified)</span></div>
        </div>
        <div style="background: rgba(15, 23, 42, 0.5); padding: 20px; border-radius: 16px; border: 1px solid #1e293b;">
            <div class="uppercase-label">Last Trained</div>
            <div style="font-size: 24px; font-weight: bold;">April 2026</div>
        </div>
        """, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
