...
def augment_realistic_urls(url_list, target_count):
    """Generate additional realistic URLs by perturbing the given real ones."""
    ...
    while len(augmented) < target_count:
        ...
        perturbed = base
        # Add query parameter if none exists
        if '?' in perturbed:
...
# ------------------------------------------------------------
# 4. FEATURE EXTRACTION (36 features)
# ------------------------------------------------------------
SUSPICIOUS_TLDS = {
...
def extract_features(url: str) -> dict:
    ...
    f['domain_age_days']   = 365   # placeholder
    return f
...
def train_model():
    ...
    y = df['label'].values
    # Replace missing domain_age_days (-1) with median
...
# ------------------------------------------------------------
# 6. PREDICTION FUNCTION (with verdict & signals)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 7. STREAMLIT UI – SENTINEL STYLE (dark, minimal, no performance tab)
# ------------------------------------------------------------
st.set_page_config(page_title="Sentinel – Malicious URL Detector", page_icon="🛡️", layout="wide")

# Custom CSS for dark, Sentinel look
st.markdown("""
<style>
...
h1 { font-size: 2.2rem !important; font-weight: 600 !important; letter-spacing: -0.02em; margin-bottom: 0.25rem; }
...
# Initialise session state for scan history
...
st.markdown('<div class="subhead">Inspect URL · Professional URL Shield</div>', unsafe_allow_html=True)
...
if analyze_clicked and url_input:
    ...
    # Graph explaining why – horizontal bar of top suspicious signals
    signals = result['signals']
    bad_signals = [s for s, t in signals if t == 'bad']
    good_signals = [s for s, t in signals if t == 'good']
    fig, ax = plt.subplots(figsize=(6, 3))
    if bad_signals:
        ax.barh(bad_signals[:5], [1]*min(5,len(bad_signals)), color='#f44336', alpha=0.7, label='Suspicious')
    if good_signals:
        ax.barh(good_signals[:3], [1]*min(3,len(good_signals)), color='#4caf50', alpha=0.7, label='Safe indicators')
